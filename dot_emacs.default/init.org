...
#+END_SRC
*** Dired
Stale block completely tied to the deprecated dired-details
   #+BEGIN_SRC emacs-lisp :tangle no
...
#+BEGIN_SRC emacs-lisp
  (use-package json-mode
    :ensure t
    :config
    (defun amchelle/config-json-mode ()
       (make-local-variable 'js-indent-level)
       (setq js-indent-level 2))
    (add-hook 'json-mode-hook 'amchelle/config-json-mode))
#+END_SRC
*** Typescript config
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package tide
    ...
    ;; Specifically for host jerry
    ;; (defun kill-tide-buffers-when-less-than-1gb-swap ()
    ;;   (when (< (string-to-number
    ;;             (shell-command-to-string "grep Swap /proc/meminfo | tail -n1 | awk '{print $2}'"))
    ;;            1000000
    ;;            )
    ;;     (message "Swap is low, killing all tide-servers")
    ;;     (kill-matching-buffers "tide-server" nil t)))
    ;;
    ;; (defvar idle-timer-kill-tide-buffers nil)
    ;;
    ;; (defun cancel-kill-tide-buffers-when-less-than-1gb-swap ()
    ;;   (interactive)
    ;;   (when (not (eq idle-timer-kill-tide-buffers nil))
    ;;     (cancel-timer idle-timer-kill-tide-buffers)
    ;;     (setq idle-timer-kill-tide-buffers nil)))
    ;; ;;

    (flycheck-add-mode 'javascript-eslint 'typescript-mode)
    (flycheck-add-mode 'javascript-eslint 'web-mode)
    (defun tide/configure-tide-typescript ()
      (bind-key "M-R" 'tide-restart-server)
      ...
      ;; after 5 minutes from now, and every 5 minutes after that
      ;; (when (eq idle-timer-kill-tide-buffers nil)
      ;;   (setq idle-timer-kill-tide-buffers
      ;;         (run-with-timer 300 300 'kill-tide-buffers-when-less-than-1gb-swap))
      ;;   )
      )
